...
export default {
  ...
  methods: {
    ...
    handleMouseDown(e, column) {
      ...
      this.dragState.start = parseInt(column.columnKey, 10)
      // 给拖动时的虚拟容器添加宽高
    ...
    handleMouseMove(e, column) {
      if (this.dragState.dragging) {
        const index = parseInt(column.columnKey, 10) // 记录起始列
        if (index - this.dragState.start !== 0) {
          this.dragState.direction = index - this.dragState.start < 0 ? 'left' : 'right' // 判断拖动方向
          this.dragState.end = parseInt(column.columnKey, 10)
        } else {
